package blobs.molecules.objects
...
/**
  ...
  */
class Cluster(containerWidth: Double,
  ...
  def move_accel(x: Double, y: Double, vX: Double, vY: Double, refParticle: Particle): Unit = {
    ...
    def withinBounds(): Boolean = {
      if (x <= upperXBound.value && x >= lowerXBound && y <= upperYBound.value && y >= lowerYBound) {
        ...
        def companionLoop(i: Int): Boolean = {
          if (i == refParticle.companions.length) true
          else {
            val p: Particle = refParticle.companions(i)
            if (p.getX <= upperXBound.value && p.getX >= lowerXBound && p.getY <= upperYBound.value && p.getY >= lowerYBound) {
              companionLoop(i+1)
            } else false
          }
        }
    ...
    def isOverlapping(p: Particle): Boolean = {
      ...
      if (!p.equals(refParticle) && distance <= 2 * particleRadius) true
        ...
        refParticle.parent = p
        refParticle.stuck = true
        p.companions = p.companions :+ refParticle
        false
      }
    }
...
class Particle(var xCoord: Double, var yCoord: Double, var color:Color = Color.Gray) {
  ...
  var parent:Particle = _
  var companions: Array[Particle] = Array()
